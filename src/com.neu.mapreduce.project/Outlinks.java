 ...
 */
public class Outlinks extends Configured implements Tool {

  private static final Logger LOG = Logger.getLogger(Outlinks.class);

  /**
   * Mapping class that produces statistics about the Common Crawl corpus.
   */
  public static class ExampleMetadataStatsMapper extends MapReduceBase implements
  ...
  public int run(String[] args) throws Exception {
    ...
    /*
     * fs = FileSystem.get(new URI("s3n://aws-publicdatasets"), job); for (FileStatus fileStatus :
     * fs.globStatus(new Path( "/common-crawl/parse-output/valid_segments/[0-9]*"))) { String[]
     * parts = fileStatus.getPath().toString().split("/"); String inputPath = baseInputPath + "/" +
     * parts[parts.length - 1] + "/metadata-*"; LOG.info("adding input path '" + inputPath + "'");
     * FileInputFormat.addInputPath(job, new Path(inputPath)); }
     */
    ...
    job.setMapperClass(ExampleMetadataStatsMapper.class);
  ...
  public static void main(String[] args) throws Exception {
    ...
    int res = ToolRunner.run(new Configuration(), new Outlinks(), args);
    System.exit(res);
  }
}
